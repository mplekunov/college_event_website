        ...
        if (parameters?.has('userID')) {
            ...
            return new Promise((resolve, reject) => {
                this.mysqlPool.getConnection((err, connection) => {
                    ...
                    connection.query(`SELECT * FROM RSO WHERE name LIKE '%${query}%' AND universityID="${parameters?.get("universityID").toString()}";`, (error, results, fields) => {
                        connection.release();
    ...
    Create(object: IBaseRSO): Promise<IRSO | null> {
        return new Promise((resolve, reject) => {
            this.mysqlPool.getConnection(async (err, connection) => {
                ...
                connection.query(`INSERT INTO RSO (rsoID, name, description, universityID) 
                VALUES ('${rsoID.toString()}', '${object.name}', '${object.description}', '${object.universityID}');`,

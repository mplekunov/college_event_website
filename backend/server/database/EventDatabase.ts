...
export default class EventDatabase implements IDatabase<IBaseEvent, IEvent> {
    ...
    GetAll(parameters?: Map<String, any> | undefined): Promise<Promise<IEvent | null>[] | null> {
        ...
        if (parameters?.get("hostType") === HostType.PUBLIC) {
            ...
            query = `hostType = ${HostType.UNIVERSITY} AND hostID = '${parameters.get("hostID")}'`;
        } else if (parameters?.get("hostType") === HostType.RSO) {
            parameters.get("hostID").forEach((hostID: any) => {
    ...
    Create(object: IBaseEvent): Promise<IEvent | null> {
        return new Promise((resolve, reject) => {
            this.mysqlPool.getConnection(async (err, connection) => {
                ...
                let location = await EventDatabase.locationDatabase.Get(new Map([["longitude", object.location.longitude], ["latitude", object.location.latitude]]));
                ...
                let events = await(new Promise((resolve, reject) => {
                    this.mysqlPool.getConnection((err, connection) => {
                        ...
                        connection.query(`SELECT * FROM Event WHERE locationID='${location?.locationID.toString()}'`, async (err, results) => {
                            connection.release();
                            ...
                            for (const result of results) {
                                events.push(await this.parseEvent(result));                            
                            }
                ...
                if (Array.isArray(events) && events.length > 0) {
                    let any = events.some((event) => {
                        return event.date === object.date;
                    });

                    if (any) {
                        return reject("Event already exists at the specified place and specified time.");
                    }
                }
